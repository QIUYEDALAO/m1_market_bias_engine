#!/usr/bin/env python3
"""Audit Phase 7A stable/watch stability candidates for research-only review."""
...
BUCKET_CSV = REPO_ROOT / "data" / "processed" / "backtest_ready" / "m1_bucket_results.csv"
FEATURE_CSV = REPO_ROOT / "data" / "processed" / "market_features" / "m1_market_features.csv"
REPORT_DIR = REPO_ROOT / "reports" / "backtest"
REPORT_MD = REPORT_DIR / "m1_stability_candidate_audit.md"
REPORT_JSON = REPORT_DIR / "m1_stability_candidate_audit.json"

EXPECTED_STABLE_COUNT = 1
EXPECTED_WATCH_COUNT = 1
EXPECTED_POSITIVE_ROI_BUCKET_COUNT = 25
EXPECTED_FEATURE_ROWS = 10733
...
def self_check(report: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        "stable_count_1": stats["stable_count"] == EXPECTED_STABLE_COUNT,
        "watch_count_1": stats["watch_count"] == EXPECTED_WATCH_COUNT,
        "positive_roi_bucket_count_25": stats["positive_roi_bucket_count"]
        == EXPECTED_POSITIVE_ROI_BUCKET_COUNT,
        "candidate_audit_count_2": stats["candidate_audit_count"] == 2,
        "input_feature_rows_10733": stats["input_feature_rows"] == EXPECTED_FEATURE_ROWS,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "This audit explains stable/watch buckets for research review only. "
            "It is not PLAY, not a recommendation, and not a current-match matching layer.",
            "",
            "## Summary",
            "",
            markdown_table(
...
def main() -> int:
    ...
    stats = report["stats"]
    print(f"m1_stability_candidate_audit: {report['self_check']['status']}")
