#!/usr/bin/env python3
"""Run offline M1 Round2 bucket backtests on complete historical feature rows only."""
...
INPUT_CSV = REPO_ROOT / "data" / "processed" / "market_features" / "m1_round2_market_features.csv"
NORMALIZED_CSV = REPO_ROOT / "data" / "processed" / "normalized_matches" / "m1_round2_matches_normalized.csv"
OUTPUT_CSV = REPO_ROOT / "data" / "processed" / "backtest_ready" / "m1_round2_bucket_results.csv"
REPORT_DIR = REPO_ROOT / "reports" / "backtest"
REPORT_MD = REPORT_DIR / "m1_round2_bucket_backtest_report.md"
REPORT_JSON = REPORT_DIR / "m1_round2_bucket_backtest_report.json"

EXPECTED_INPUT_ROWS = 9063
EXPECTED_BACKTEST_ROWS = 7606
EXPECTED_EXCLUDED_CURRENT_ROWS = 1457
...
def self_check(input_rows: list[dict[str, str]], backtest_rows: list[dict[str, str]], result_rows: list[dict[str, str]], stats: dict[str, Any]) -> dict[str, Any]:
    checks = {
        "input_rows_9063": stats["input_rows"] == EXPECTED_INPUT_ROWS,
        "backtest_rows_7606": stats["backtest_rows"] == EXPECTED_BACKTEST_ROWS,
        "excluded_current_rows_1457": stats["excluded_current_rows"] == EXPECTED_EXCLUDED_CURRENT_ROWS,
...
def render_markdown(report: dict[str, Any], result_rows: list[dict[str, str]]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "This is an offline Round2 bucket backtest over `COMPLETE_BACKTEST` rows only. "
            "It does not merge Round1 data, match current fixtures, generate recommendations, "
            "or write official/pending/QQ outputs.",
            "",
...
def main() -> int:
    ...
    print(f"m1_round2_bucket_backtest: {checks['status']}")
