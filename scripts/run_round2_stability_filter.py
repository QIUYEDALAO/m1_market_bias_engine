#!/usr/bin/env python3
"""Apply Round2 Phase 5 stability filters to M1 offline bucket backtest results."""
...
INPUT_CSV = REPO_ROOT / "data" / "processed" / "backtest_ready" / "m1_round2_bucket_results.csv"
INPUT_REPORT = REPO_ROOT / "reports" / "backtest" / "m1_round2_bucket_backtest_report.json"
THRESHOLDS_PATH = REPO_ROOT / "config" / "round2_stability_thresholds.json"
OUTPUT_CSV = REPO_ROOT / "data" / "processed" / "backtest_ready" / "m1_round2_stability_filtered_buckets.csv"
REPORT_DIR = REPO_ROOT / "reports" / "backtest"
REPORT_MD = REPORT_DIR / "m1_round2_stability_filter_report.md"
REPORT_JSON = REPORT_DIR / "m1_round2_stability_filter_report.json"
...
def self_check(
    ...
    checks = {
        ...
        "phase4_backtest_rows_7606": input_report["stats"]["backtest_rows"] == 7606,
        "phase4_excluded_current_rows_1457": input_report["stats"]["excluded_current_rows"]
        == 1457,
...
def render_markdown(report: dict[str, Any], output_rows: list[dict[str, str]]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "This phase filters Round2 offline bucket results only. It does not merge "
            "Round1, match current fixtures, generate recommendations, write "
            "official/pending/QQ outputs, or allow five-dimension evaluation to create candidates.",
            "",
            "## Status Counts",
            "",
            markdown_table(
...
def main() -> int:
    ...
    print(f"m1_round2_stability_filter: {checks['status']}")
