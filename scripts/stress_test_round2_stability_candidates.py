#!/usr/bin/env python3
"""Stress test Round2 stability candidates across seasons and leagues."""
...
CANDIDATE_AUDIT_JSON = REPO_ROOT / "reports" / "backtest" / "m1_round2_stability_candidate_audit.json"
...
REPORT_MD = REPORT_DIR / "m1_round2_stability_candidate_stress_test.md"
REPORT_JSON = REPORT_DIR / "m1_round2_stability_candidate_stress_test.json"
...
LEAGUE_GROUPS = {
    "group_a_n1_b1_p1": {"N1", "B1", "P1"},
    "group_b_t1_sc0": {"T1", "SC0"},
}
...
def row_matches_candidate(row: dict[str, str], candidate: dict[str, str]) -> bool:
    ...
    if candidate["market"] == "1X2" and bucket_type == "closing_fair_probability":
        field = {
    ...
    if candidate["market"] == "1X2" and bucket_type == "open_to_close_odds_movement":
        field = {
    ...
    if candidate["market"] == "AH" and bucket_type == "closing_line":
        return ah_line_bucket(to_float(row.get("closing_ah_line", ""))) == bucket
    return False
...
def ah_home_profit(row: dict[str, str]) -> tuple[float | None, float | None, float | None]:
    odds = to_float(row.get("closing_ah_home_odds", ""))
    settlement = row.get("ah_settlement", "")
    payout = {
        "FULL_WIN": 1.0,
        "HALF_WIN": 0.5,
        "PUSH": 0.0,
    ...
    if payout > 0:
        ...
        profit = 0.0
    return profit, odds, 1.0 if payout > 0 else 0.0
...
def slice_metrics(rows: list[dict[str, str]], candidate: dict[str, str]) -> dict[str, Any]:
    profits = []
    hit_count = 0.0
    odds_values = []
    selection = candidate["selection"]
    for row in rows:
        if candidate["market"] == "AH":
            profit, odds, hit = ah_home_profit(row)
            if profit is None or odds is None or hit is None:
                continue
...
def build_slices(candidate: dict[str, str], rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for slice_type, slice_name, slice_rows, critical in specs:
        metrics = slice_metrics(slice_rows, candidate)
...
def self_check(report: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "candidate_count_3": stats["candidate_count"] == 3,
        ...
        "feature_rows_9063": stats["feature_rows"] == 9063,
        "complete_rows_7606": stats["complete_backtest_feature_rows"] == 7606,
        "current_rows_excluded_1457": stats["current_not_for_backtest_feature_rows"] == 1457,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- normalized_csv: `{report['inputs']['normalized_csv']}`",
            ...
            "",
            "This is a Round2 research-only stress test. SURVIVED does not mean PLAY; "
...
def main() -> int:
    ...
    for path in [stability_csv, feature_csv, normalized_csv, candidate_audit_json]:
        if not path.exists():
            raise SystemExit(f"Required input does not exist: {path}")

    stability_rows = read_csv(stability_csv)
    feature_rows = enrich_with_normalized_fields(read_csv(feature_csv), read_csv(normalized_csv))
    _candidate_audit = read_json(candidate_audit_json)
    report = build_report(stability_csv, feature_csv, normalized_csv, candidate_audit_json, stability_rows, feature_rows)
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    REPORT_JSON.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_MD.write_text(render_markdown(report), encoding="utf-8")

    stats = report["stats"]
    print(f"m1_round2_stability_candidate_stress_test: {report['self_check']['status']}")
